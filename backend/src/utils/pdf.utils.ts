...
export class PDFUtils {
  static async generateScorecard(matchData: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      if (matchData.tossWinner) {
        doc.text(`Toss: ${matchData.tossWinner} won and chose to ${matchData.tossDecision}`,
                 doc.page.width / 2, infoY + 30);
      }
      ...
      if (matchData.innings && matchData.innings.length > 0) {
        matchData.innings.forEach((innings: any, index: number) => {
          if (currentY > 700) {
            doc.addPage();
            currentY = 50;
          }
          ...
          doc.fontSize(12)
             .text(`${innings.totalRuns}/${innings.totalWickets} (${innings.totalOvers} overs)`,
                   doc.page.width - 200, currentY + 10);
          ...
          if (innings.battingPerformances) {
            innings.battingPerformances.forEach((bat: any, idx: number) => {
          ...
          if (innings.extras) {
            doc.fontSize(9)
               .font('Helvetica-Bold')
               .text(`Extras: ${innings.extras} (W: ${innings.wides || 0}, NB: ${innings.noBalls || 0}, B: ${innings.byes || 0}, LB: ${innings.legByes || 0})`,
                     60, currentY);
            currentY += 20;
          }
          ...
          if (innings.bowlingPerformances) {
            innings.bowlingPerformances.forEach((bowl: any, idx: number) => {
              const bgColor = idx % 2 === 0 ? '#ffffff' : lightGray;
              doc.rect(50, currentY, doc.page.width - 100, 20).fill(bgColor);
  ...
  static async generateTournamentReport(tournamentData: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      const doc = new PDFDocument({ margin: 50, size: 'A4' });
      const buffers: Buffer[] = [];
      ...
      doc.text(`End Date: ${new Date(tournamentData.endDate).toLocaleDateString()}`,
               doc.page.width / 2, currentY + 60);
      ...
      if (tournamentData.pointsTable) {
        tournamentData.pointsTable.forEach((team: any, idx: number) => {
          const bgColor = idx % 2 === 0 ? '#ffffff' : lightGray;
          doc.rect(50, currentY, doc.page.width - 100, 20).fill(bgColor);
          doc.fillColor('#000000')
             .fontSize(9)
      ...
      if (tournamentData.topBatsmen || tournamentData.topBowlers) {
        ...
        if (tournamentData.topBatsmen) {
          ...
          tournamentData.topBatsmen.slice(0, 5).forEach((batsman: any) => {
            doc.fontSize(10)
               .font('Helvetica')
        ...
        if (tournamentData.topBowlers) {
          ...
          tournamentData.topBowlers.slice(0, 5).forEach((bowler: any) => {
            doc.fontSize(10)
               .font('Helvetica')
