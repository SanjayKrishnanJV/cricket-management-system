 ...
 * Handles Video Highlights, Live Streaming, Podcasts, and Broadcaster Dashboard
 */
export class BroadcastService {
  ...
  async createVideoHighlight(data: {
    ...
    return await prisma.videoHighlight.create({
      data: {
        ...data,
        videoProvider,
        videoId,
  ...
  async getVideoHighlights(
    ...
    return await prisma.videoHighlight.findMany({
      where: {
        matchId,
  ...
  async linkBallToVideo(ballId: string, videoId: string, timestamp: number) {
    ...
    return await prisma.videoHighlight.update({
      where: { id: videoId },
      data: {
        ballId,
  ...
  async autoGenerateHighlights(matchId: string) {
        ...
        innings: {
          include: {
            balls: {
              where: {
                OR: [
                  { runs: { gte: 4 } }, // Boundaries
                  { isWicket: true },   // Wickets
                ],
              },
              include: {
    ...
    if (!match.liveStream || !match.liveStream.streamUrl) {
      throw new AppError('No live stream configured for this match', 400);
    }
    ...
    const result = await prisma.videoHighlight.createMany({
      data: highlights,
      skipDuplicates: true,
    });
  ...
  async setupLiveStream(
    ...
    if (existingStream) {
      throw new AppError('Live stream already configured for this match', 400);
    }
  ...
  async updateStreamStatus(matchId: string, status: string) {
  ...
  async updateStreamAnalytics(matchId: string, viewers: number) {
    const stream = await prisma.liveStream.findUnique({
      where: { matchId },
    });
  ...
  async generateMatchPodcast(
    ...
    if (match.status !== 'COMPLETED') {
      throw new AppError('Cannot generate podcast for incomplete match', 400);
    }
    ...
    const podcast = await prisma.matchPodcast.create({
      data: {
        matchId,
        ...
        audioUrl: '', // Will be updated after TTS generation
        audioFormat: 'mp3',
    ...
    // TODO: Trigger async job to generate actual audio using AWS Polly
    // For now, mark as READY with a placeholder URL
    // In production, you would use a job queue (Bull/BullMQ) to process this asynchronously

    return podcast;
  }

  async getPodcastStatus(podcastId: string) {
    return await prisma.matchPodcast.findUnique({
      where: { id: podcastId },
      include: {
        match: {
          select: {
  ...
  async publishPodcast(podcastId: string) {
    const podcast = await prisma.matchPodcast.findUnique({
      where: { id: podcastId },
    });
    ...
    if (podcast.status !== 'READY') {
      throw new AppError('Podcast is not ready for publishing', 400);
    }

    return await prisma.matchPodcast.update({
      where: { id: podcastId },
      data: {
        isPublished: true,
  ...
  async getMatchPodcasts(matchId: string) {
    return await prisma.matchPodcast.findMany({
      where: { matchId },
      orderBy: { createdAt: 'desc' },
    });
  }
  ...
  async getBroadcasterSettings(matchId: string) {
    let settings = await prisma.broadcasterSettings.findUnique({
      where: { matchId },
    });

    if (!settings) {
      settings = await prisma.broadcasterSettings.create({
        data: { matchId },
      });
    }
  ...
  async updateBroadcasterSettings(matchId: string, updates: any) {
    return await prisma.broadcasterSettings.upsert({
      where: { matchId },
      update: updates,
      create: {
        matchId,
  ...
  async generateTalkingPoints(matchId: string, overNumber?: number) {
    const match = await prisma.match.findUnique({
      where: { id: matchId },
      include: {
        homeTeam: { select: { name: true } },
    ...
    // Economy rate extreme
    const expensiveBowler = innings.bowlingPerformances.find(
      (b) => b.economyRate && b.economyRate > 12
    );
    if (expensiveBowler) {
      points.push({
        matchId,
    ...
    if (innings.inningsNumber === 2 && match.innings.length > 1) {
      ...
      const ballsLeft = (20 * 6) - (innings.totalOvers * 6); // Assuming T20
    ...
    // Create talking points in database
    if (points.length > 0) {
    ...
    // Return unused talking points sorted by priority
    return await prisma.talkingPoint.findMany({
  ...
  async getTalkingPoints(matchId: string) {
    return await prisma.talkingPoint.findMany({
      where: { matchId, isUsed: false },
      orderBy: { priority: 'desc' },
    });
  ...
  async markTalkingPointUsed(pointId: string) {
    return await prisma.talkingPoint.update({
      where: { id: pointId },
  ...
  private detectProvider(url: string): string {
    ...
    if (url.includes('s3.amazonaws.com') || url.includes('cloudfront.net')) return 's3';
    return 'custom';
  }
  ...
  private calculateTimestamp(overNumber: number, ballNumber: number): number {
    // Rough estimate: each over takes ~3 minutes (180 seconds)
    const overTime = overNumber * 180;
    const ballTime = ballNumber * 30;
    return overTime + ballTime;
  }

  private generatePodcastScript(match: any): string {
    const script = `
...
Thank you for listening to this match recap.
  ...
  private generatePodcastSummary(match: any): string {
    return `${match.homeTeam.name} vs ${match.awayTeam.name} at ${match.venue}. ${match.resultText}`;
  }

  private generateKeyMoments(match: any): Array<{ time: number; title: string }> {
    const moments: Array<{ time: number; title: string }> = [];
  ...
  private generateChapters(match: any): Array<{ time: number; title: string }> {
    return [
